import { create } from "zustand";
import axios from "axios";
import toast from "react-hot-toast";
import api from "../api";
export const useEventStore = create((set, get) => ({
  filter: "all",
  filterdEvents: [],
  events: [],
  Attendances: [],
  loading: false,
  error: null,
  setFilter: (filter) => set({ filter }),
  formData: {
    created_by_member_id: "",
    type: "general",
    title: "",
    description: "",
    location: "",
    start_datetime: "",
    end_datetime: "",
    point_value: 0,
    poster_img_file: null,
    poster_img_url:
      "https://png.pngtree.com/png-clipart/20201209/original/pngtree-event-location-icon-png-image_5656143.jpg",
    max_attendee: false,
  },
  setFormData: (formData) => set({ formData }),
  resetFormData: () =>
    set({
      formData: {
        created_by_member_id: "",
        type: "general",
        title: "",
        description: "",
        location: "",
        start_datetime: "",
        end_datetime: "",
        point_value: 0,
        poster_img_file: null,
        poster_img_url:
          "https://png.pngtree.com/png-clipart/20201209/original/pngtree-event-location-icon-png-image_5656143.jpg",
        max_attendee: false,
      },
    }),
  fetchEvents: async () => {
    set({ loading: true, error: null });
    try {
      const response = await api.get(`/events`);
      set({
        events: response.data.data,
        error: null,
        filteredEvents: response.data.data,
      });
      console.log("fecthed events: ", response.data.data);
    } catch (err) {
      console.log("error fetching events:", err);
      set({ error: err });
    } finally {
      set({ loading: false });
    }
  },
  fetchEvent: async (id) => {
    set({ loading: true });
    const { setFormData, formData } = get();
    try {
      const response = await api.get(`/events/${id}`);
      setFormData(response.data.data);
      console.log(formData);
    } catch (err) {
      console.log(err);
    } finally {
      set({ loading: false });
    }
  },
  filterEvents: () => {
    set({ loading: true });
    const { filter, events } = get();
    if (filter === "all") {
      set({ filteredEvents: events });
      set({ loading: false });
    } else if (filter === "upcoming") {
      const upcomingEvents = events.filter(
        (e) => new Date(e.start_datetime) > new Date()
      );
      set({ filteredEvents: upcomingEvents });
      set({ loading: false });
    } else if (filter === "ongoing") {
      const ongoingEvents = events.filter(
        (e) =>
          new Date(e.start_datetime).toISOString().slice(0, 10) ===
          new Date().toISOString().slice(0, 10)
      );
      set({ filteredEvents: ongoingEvents });
      set({ loading: false });
    } else if (filter === "past") {
      const pastEvents = events.filter(
        (e) =>
          new Date(e.start_datetime) < new Date() &&
          !(
            new Date(e.start_datetime).toISOString().slice(0, 10) ===
            new Date().toISOString().slice(0, 10)
          )
      );
      set({ filteredEvents: pastEvents });
      set({ loading: false });
    }
  },
  getAttendance: async () => {
    set({ loading: true });
    try {
      const results = await api.get("/attendances");
      console.log(results.data.data);
    } catch (err) {
      console.log("error getting attendances: ", err);
      set({});
    } finally {
      set({ loading: false });
    }
  },
  getAttendanceByMemberId: async (id) => {
    set({ loading: true });
    try {
      const response = await addPoints.get(`attendances/member/${id}`);
      set({ attendances: response.data.data, error: null });
    } catch (err) {
      console.log("error get an attendance: ", err);
    } finally {
      set({ loading: false });
    }
  },
  createEvent: async () => {
    set({ loading: true });
    const { formData } = get();
    var data = new FormData();
    for (const key in formData) {
      data.append(key, formData[key]);
    }
    try {
      const response = await api.post(`/events/`, data);
      if (response.data.data) toast.success("event added successfully");
    } catch (err) {
      console.log("error adding event", err);
      toast.error("error adding event, try again");
    } finally {
      set({ loading: false });
    }
  },
  updateEvent: async (id) => {
    set({ loading: true });
    const { formData } = get();
    var data = new FormData();
    for (const key in formData) {
      data.append(key, formData[key]);
    }
    try {
      const response = await api.put(`/events/${id}`, data);
      if (response.data.data) toast.success("updated successfully");
    } catch (err) {
      console.log("error updating event", err);
      toast.error("error updating event, try again");
    } finally {
      set({ loading: false });
    }
  },
  deleteEvent: async(id)=> {
      set({loading:true})
      try{
        const res =  await api.delete(`events/${id}`)
        if (res) toast.success("event deleted successfully")
      }catch(err){
        console.log("error: ", err)
        toast.error("error deleting event")
      }finally{
        set({loading: false})
      }

  }
}));
